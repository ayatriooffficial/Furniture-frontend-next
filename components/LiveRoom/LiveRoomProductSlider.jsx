...
const LiveRoomProductSlider = ({ products }) => {
  ...
  useEffect(() => {
    ...
    products?.forEach((item) => {
      if (item.specialprice?.startDate && item.specialprice?.endDate) {
        const startDate = new Date(item.specialprice.startDate);
        const endDate = new Date(item.specialprice.endDate);
        ...
        >
          {!products ? (
            ...
            products.map((item,index) => (
              <SwiperSlide key={`${item._id}_${index}`}>
                          ...
                          <div>
                            <p className="text-sm font-semibold bg-yellow-400 price-box shadow-[3px_3px_rgb(173,_53,_53)] w-fit px-2 py-1">
                              ...
                              <span className="text-3xl">
                                {Math.floor(item.specialprice?.price)}
                              </span>
                            </p>
                            <p className="text-sm mt-2 text-gray-500">
                              Regular price: Rs.{Math.floor(item.perUnitPrice)}
                            </p>
                            {item.specialprice.startDate &&
                              item.specialprice.endDate &&
                              formattedDates[item._id] && (
                                <p className="text-sm mt-1 text-gray-500">
                                  Price valid from{" "}
                                  {formattedDates[item._id].startDate} to{" "}
                                  {formattedDates[item._id].endDate}
                                </p>
                              )}
                          </div>
                        ) : (
